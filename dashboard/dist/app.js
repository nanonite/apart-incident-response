...
const CREATIVE_TASKS = ['poetry-duet','civic-law-1943'];
...
function render(){
 ...
 const active=Boolean(d.can_stop);const unresolved=(d.batches||[]).some(x=>x.status==='running');const staleServer=ui.live&&d.server?.api_version!=='response-panel-v7';$('start').disabled=!ui.live||active||unresolved||staleServer;$('stop').hidden=!d.can_stop;
 ...
 const task=knownTasks.find(t=>t.id===ui.task)||{};$('task-category').textContent=task.category||'';$('task-title').textContent=task.title||'Choose a task';$('task-question').textContent=task.question||'';$('difficulty').textContent=`Difficulty ${task.difficulty||'?'} / 5`;const roles=task.agent_roles?` Roles: A=${task.agent_roles.A||'solver'}, B=${task.agent_roles.B||'solver'}.` : ''; $('task-design').textContent=(task.design||'')+roles;
...
function selectedTaskIds(){const value=$('task-select').value;return value==='all'?ui.data.tasks.map(t=>t.id):value==='asymmetric'?ASYMMETRIC_TASKS:value==='creative'?CREATIVE_TASKS:value==='experiment1'?['locked-database']:[value];}
function estimate(){const value=$('task-select').value;const count=value==='all'?(ui.data?.tasks?.length||9):value==='asymmetric'?ASYMMETRIC_TASKS.length:value==='creative'?CREATIVE_TASKS.length:1;const n=count*$('conditions').value.split(',').length*Number($('steps').value)*Number($('repeats').value)*2;$('run-estimate').textContent=`${n} requested updates · ${$('steps').value} checkpoints × ${$('repeats').value} repeats · ${$('adapter').value==='fixture'?'fixture, no LLM':'local inference'}`;}
$('run-form').onchange=estimate;
$('task-select').onchange=()=>{const creative=$('task-select').value==='creative';if(creative){$('conditions').value='C0,C2';$('peer-bytes').value='6000';$('shared-context').value='full_history';$('engagement').value='peer_review';notice('Creative pilot defaults applied: C0/C2, 6 KiB whole-message history budget, and peer review.');}estimate();};
function requestedConfig(){const taskIds=selectedTaskIds();return {task_ids:taskIds,conditions:$('conditions').value.split(','),steps:Number($('steps').value),repeats:Number($('repeats').value),unlock_step:Number($('unlock').value),adapter:$('adapter').value,model:$('model').value,logprobs:$('logprobs').checked,engagement_mode:$('engagement').value,shared_context_mode:$('shared-context').value,
 deadline_seconds:Number($('run-minutes').value||15)*60,request_timeout_seconds:Number($('request-seconds').value||120),batch_timeout_seconds:Number($('batch-minutes').value||60)*60,max_output_tokens:Number($('output-tokens').value||512),
 communication_budget_bytes:$('peer-bytes').value?Number($('peer-bytes').value):null,communication_delay_steps:Number($('peer-delay').value||0),communication_cost_per_kib:Number($('peer-cost').value||0),communication_request_required:Boolean($('peer-request').checked),evidence_mode:$('evidence-mode').value||'split',role_mode:$('role-mode').value||'specialized',role_swap_step:$('role-swap').value?Number($('role-swap').value):null,
 schedule:'synchronous_snapshot_serial_inference',study_id:$('pilot-preset').value==='calibration'?'collaboration_calibration_v1':taskIds.includes('locked-database')?'locked_database_v1':'collaboration_study_v1'};}
$('pilot-preset').onchange=()=>{if($('pilot-preset').value==='calibration'){for(const [id,value] of Object.entries({'task-select':'sensor-fusion',conditions:'C2',steps:'5',repeats:'2',unlock:'3','run-minutes':'15','request-seconds':'120','batch-minutes':'31','output-tokens':'512','peer-bytes':'','peer-delay':'0','peer-cost':'0','role-swap':'','evidence-mode':'split',engagement:'neutral','shared-context':'key_insights_plus_history'}))$(id).value=value;$('peer-request').checked=false;notice('Calibration configured. Run Fixture first, then Ollama. These are separate labeled batches.');estimate();}};
$('run-form').onsubmit=async e=>{e.preventDefault();try{const config=requestedConfig();const data=await post('/api/run',config);ui.batch=data.batch_id;ui.task=config.task_ids[0];ui.step=0;ui.repeat=0;notice('Batch started. Checkpoints, roles, delivery costs, and timing are being recorded.');setTimeout(load,200);}catch(err){notice(err.message);}};
