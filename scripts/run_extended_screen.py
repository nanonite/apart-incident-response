"""ISO/FULL/COMM extended screen — four additional families (#117-120).

Pre-registration (recorded before any live calls):
  Anchor model: ling (inclusionai/ling-3.0-flash-vl:free)
  Families: planning, poetry, legal, lexicon
  ...
  Seeds: 101, 102 per cell (same as anchor screen for cross-family comparison)
...
Artifacts: runs/discovery-145/extended_screen/ (turn-1 files are retained as
...
SEEDS = [101, 102]
ARTIFACT_DIR = _repo_root / "runs" / "discovery-145" / "extended_screen"
...
def run() -> dict:
    ...
    print("ISO/FULL/COMM extended screen — pre-registered parameters")
