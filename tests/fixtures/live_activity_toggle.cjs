...
assert.match(elements.get('live-state-B').textContent,/worker unverified/);
data.execution={state:'verified_worker'};
data.live_activity.agents.B.state_since=new Date(Date.now()-61*60*1000).toISOString();
render();
